...
#if ESPHOME_LOG_LEVEL >= ESPHOME_LOG_LEVEL_VERBOSE
  ESP_LOGV(TAG, "Wrote GPIO modes: 0b" BYTE_TO_BINARY_PATTERN, BYTE_TO_BINARY(this->mode_mask_));
  ESP_LOGV(TAG, "Wrote GPIO pullup/pulldown: 0b" BYTE_TO_BINARY_PATTERN, BYTE_TO_BINARY(this->pull_up_down_mask_));
  ESP_LOGV(TAG, "Wrote GPIO pull enable: 0b" BYTE_TO_BINARY_PATTERN, BYTE_TO_BINARY(this->pull_enable_mask_));
#endif
  this->status_clear_warning();
  return true;
}
